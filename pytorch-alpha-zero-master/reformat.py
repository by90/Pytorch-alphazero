    ...
    while len(file_names) > 0:
        ...
        while True:
            ...
            print(game,
                  file=open(
                      os.path.join(
                          new_dir, '{}_{}.pgn'.format(thread_idx,
                                                      file_name_idx)), 'w'),
                  end='\n\n')
            file_name_idx += 1
            if file_name_idx % 1000 == 0:
                print('Thread {} wrote {} train pngs'.format(
                    thread_idx, file_name_idx))
...
for i in range(num_threads):
    ...
    threads.append(
        Thread(target=reformat_games,
               args=(all_file_names[i * files_per_thread:(i + 1) *
                                    files_per_thread], reformat_dir, i)))
    threads[i].start()
    time.sleep(0.0001)
